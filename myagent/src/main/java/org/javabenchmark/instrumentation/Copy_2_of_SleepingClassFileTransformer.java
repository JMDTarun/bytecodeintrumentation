...
public class Copy_2_of_SleepingClassFileTransformer implements ClassFileTransformer {
	...
	private static Map<String, List<String>> interfacesMap = new HashMap<String, List<String>>();
	...
	static {
		...
		try {
			...
			//classes.add("org.hibernate.loader.Loader");
			if (configurations != null && configurations.getConfiguration() != null && configurations.getConfiguration().size() > 0) {
				for (Configuration configuration : configurations.getConfiguration()) {
					...
					if (configuration.getInterfaceName() != null) {
						interfacesMap.put(configuration.getInterfaceName(), configuration.getMethodName());
					}
				}
			}
			System.out.println("Location List....... " + location);
			System.out.println("Classes List....... " + classes);
			System.out.println("Interface Map....... " + interfacesMap);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		if (classes != null && classes.size() > 0 && classes.contains(className)) {
			...
			try {
				...
				try {
					...
					System.out.println("Loading.............. "+classNameStr);
					cc = cp.get(classNameStr);
				}
				...
				if(className.equals("org/hibernate/loader/Loader") || className.equals("org.hibernate.loader.Loader")) {
					System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
					CtMethod[] methods = cc.getDeclaredMethods();
					
					for (CtMethod m : methods) {
						System.out.println(m.getName());
						if(m.getName().equals("getSQLInsertStrings")) {
							m.insertBefore("for(int i=0;i<sqlInsertStrings.length; i++) {System.out.println(\"%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% \"+sqlInsertStrings[i])}");
						}
						System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! "+m.getName());
						//m.getMethodInfo().
						System.out.println(m.getModifiers());
						if(!(m.getModifiers() == 1028)) {
							try {
								//m.insertBefore("System.out.println(this.getSQLString());");
							} catch (Exception e) {
								e.printStackTrace();
							}
				...
				if (interfaces != null && interfaces.length > 0) {
					for (CtClass ctClass : interfaces) {
						...
						if (interfacesMap != null && interfacesMap.size() > 0
								&& (interfacesMap.containsKey(ctClass.getName()) || interfacesMap.containsKey(ctClass.getName().replaceAll("/", ".")))) {
							...
							if (methods != null && methods.length > 0) {
								...
								for (CtMethod m : methods) {
									...
									List<String> listMethods = interfacesMap.get(ctClass.getName());
									if (listMethods != null && listMethods.size() > 0) {
										if (listMethods.contains(m.getName())) {
						...
						// if
						// (ctClass.getName().equals("org.javabenchmark.instrumentation.CustomInterface")
						// ||
						// ctClass.getName().equals("org/springframework/data/redis/core/HashOperations")
						// ||
						// ctClass.getName().equals("org.springframework.data.redis.core.HashOperations")
						// ||
						// ctClass.getName().equals("com.mmt.solr.ISolrClient")
						// ||
						// ctClass.getName().equals("com/mmt/solr/ISolrClient")
						...
						// // }
						// // }
						// System.out.println("Class can be assigned to Custom Iterface .............. "
						// + className);
						// // if (!addedClasses.contains(ctClass.getName())) {
						// CtMethod[] methods = cc.getDeclaredMethods();
						//
						// if (methods != null && methods.length > 0) {
						// System.out.println("Method Array Length ..... " +
						// methods.length);
						// for (CtMethod m : methods) {
